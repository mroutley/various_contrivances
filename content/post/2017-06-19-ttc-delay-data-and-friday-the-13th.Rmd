...
Now we have a `delays` dataframe with `r dim(delays)[1]` incidents from `r min(delays$date)` to `r max(delays$date)`. We need to identify any days that are a Friday the 13th. We also might want to compare weekends, regular Fridays, other weekdays, and Friday the 13ths, so we add a `type` column that provides these values. Here I use the `case_when` function:
...
```{r}
model <- lm(total_delay ~ type, data = daily_delays, subset = type %in% c("Friday", "Friday 13th"))
summary(model)
```

Definitely no statistical support for the idea that Friday the 13ths cause longer TTC delays. How about time series tests, like [anomaly detections](https://blog.statsbot.co/time-series-anomaly-detection-algorithms-1cef5519aef2)? Seems like we'd just be getting carried away. Part of the art of statistics is knowing when to quit.
